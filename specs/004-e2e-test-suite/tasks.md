...
- [ ] T005 [US5] Add key-presence guard in `bureau/cli.py`: after `load_dotenv`, check `os.environ.get("ANTHROPIC_API_KEY")`; if absent, print `"[bureau] error: ANTHROPIC_API_KEY not set — add it to ~/.bureau/.env or export it in your shell"` and `sys.exit(1)`; guard must only fire on commands that invoke persona nodes (not `bureau --help` or `bureau list`)
...
- [ ] T008 [US4] Create `tests/e2e/conftest.py` with: (a) module-level `pytest.mark.skipif` constants for `BUREAU_TEST_REPO` and `ANTHROPIC_API_KEY` absence; (b) session-scoped `bureau_test_repo` fixture that reads `BUREAU_TEST_REPO`, asserts the path exists on disk, runs `git -C <path> checkout main` to reset state, and yields a `Path`; (c) `run_bureau(spec_path: str, repo_path: str) -> subprocess.CompletedProcess` helper that calls `subprocess.run([bureau_exe(), "run", spec_path, "--repo", repo_path], capture_output=True, text=True, timeout=600)`
