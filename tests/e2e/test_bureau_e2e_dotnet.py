...
from tests.e2e.conftest import SKIP_NO_DOTNET_REPO, SKIP_NO_KEY, run_bureau

pytestmark = [SKIP_NO_DOTNET_REPO, SKIP_NO_KEY]
...
def test_kafka_observability_dashboard(bureau_test_dotnet_repo):
    spec_path = str(
        Path(bureau_test_dotnet_repo) / "specs" / "001-kafka-observability-dashboard" / "spec.md"
    )
    result = run_bureau(spec_path, bureau_test_dotnet_repo, timeout=1750)
